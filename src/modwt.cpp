...
NumericMatrix modwt(NumericVector X, String filter, int J) {
  ...
  int N = X.size();
  NumericVector g_t = scaling_filter(filter);
  NumericVector h_t = wavelet_filter(filter);
  int L = g_t.size();
  ...
  for (int j = 1; j <= J; ++j) {
    ...
    for (int t = 0; t < N; ++t) {
      ...
      for (int n = 1; n < L; ++n) {
        ...
        W(t, j-1) = W(t, j-1) + h_t[n]*V_last[k];
        V[t] = V[t] + g_t[n]*V_last[k];
      }
    }
    V_last = clone(V);
  }

  NumericMatrix WV_J = cbind(W, V);
